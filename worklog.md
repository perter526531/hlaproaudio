...
---
Task ID: 18
Agent: full-stack-developer (self-contained solution-cards block)
Task: Make the home "应用领域" cards fully editable in PageManager (card images/names/descriptions stored in the block, not on categories)
Work Log:
- Read worklog.md, ContentBlock.tsx (public block renderer), PageManager.tsx (BlockDialog + structured editors), ImageUploader, admin hooks (useCategoriesAdmin), lib/types (Category), API /api/categories (returns full tree rooted at parentId===null so root === L1), and prisma/seed.ts to align with the existing stats/features/news-list editor pattern before editing.
- ContentBlock.tsx: switched the `case "solution-cards"` line to pass `contentEn={block.contentEn}` (was `content={content}`). Rewrote `SolutionCardsBlock` to take `{ title, contentEn }`: defines `interface SolutionCard { nameEn; nameCn; descEn; descCn; image; linkCategoryId? }`, parses contentEn via the existing `parseJsonArray<SolutionCard>` helper, and on null/empty array falls back to AUTO-PULLING the L1 categories via `useCategories()` mapped to SolutionCard rows (so unedited blocks keep their cards). Card click: `card.linkCategoryId ? go({name:"products", categoryId}) : go({name:"products"})`. Card rendering (absolute bg img + dark scrim `from-black/80 via-black/30 to-black/10` + name + desc + Learn-more ChevronRight) is byte-for-byte identical to before. Dropped the eyebrow entirely for this block type (the heading `title` alone is enough) so repurposing contentEn for the JSON payload doesn't lose the eyebrow text.
- PageManager.tsx: added `useCategoriesAdmin` to the imports + `Category` to the `./types` type import. Added `SolutionCardRow` type + `emptySolutionCardRow()` factory. Added `parseSolutionCards(raw, l1Categories)` (validates each field, falls back to mapping the L1 categories to rows so the admin sees what they edit — same WYSIWYG rule as stats/features/news-list; falls back to 4 empty rows when no L1 categories exist yet) and `serializeSolutionCards(rows)` (drops rows with empty nameEn AND nameCn AND no image; omits `linkCategoryId` from the JSON when empty so the public block uses the "all products" branch). Added `SolutionCardsEditor` component modeled on NewsListEditor: repeatable rows with nameEn/nameCn Inputs, descEn/descCn small Textareas, an `ImageUploader` (preview `h-24 w-40`) for the card image, and a `<Select>` listing the L1 categories (fetched via `useCategoriesAdmin`, root array IS the L1 set) plus an "All products / 全部产品" option (value `__all__` -> linkCategoryId "") for the optional per-card click target. Bilingual hint line included. Wired into BlockDialog: local `solutionCardRows` state (4 empty rows init), added `useCategoriesAdmin()` + memoized `l1Categories`, init from `parseSolutionCards(block?.contentEn, l1Categories)` in the existing init `useEffect`, re-init on type change to "solution-cards", added `updateSolutionCardRow/addSolutionCardRow/removeSolutionCardRow` mirroring the news handlers (every change syncs to `form.contentEn` via `serializeSolutionCards`). `onSave`'s `isStructured` flag now includes `solution-cards` so `contentCn` is nulled on save (bilingual payload lives entirely in contentEn). Render branch added between news-list and the default textarea.
- prisma/seed.ts: updated the home `solution-cards` block (order 4) so `contentEn` is a JSON array of 4 cards built from the L1 names/descriptions/images (Loudspeakers->IMG.prodLineArray, Amplifiers->IMG.prodAmplifier, Mixers & Processors->IMG.prodMixer, Wireless & Microphones->IMG.prodWireless). Dropped the `contentEn:"Solutions"` eyebrow + `contentCn:"解决方案"` (contentCn left null/absent per spec). linkCategoryId omitted at seed time because the home blocks are created BEFORE the L1 categories in the seed file; the admin can wire the per-card click target later via PageManager. Added an inline comment explaining the ordering constraint.
- Ran `bun run prisma/seed.ts` -> "Seed complete. Admin: admin / admin123". Ran `bun run lint` -> 0 errors, 0 warnings (after removing one unnecessary eslint-disable directive that was no longer needed). Dev server on :3000 hot-reloaded the changes; no runtime errors in dev.log.

Stage Summary:
- Files changed: src/components/public/ContentBlock.tsx (SolutionCardsBlock now self-contained JSON-card renderer with L1 fallback + dropped eyebrow; switch passes contentEn), src/components/admin/PageManager.tsx (new SolutionCardsEditor + parse/serialize helpers + BlockDialog wiring; useCategoriesAdmin + Category type added to imports), prisma/seed.ts (home solution-cards block contentEn is now a 4-card JSON array, eyebrow dropped).
- Key decision: DROP the eyebrow for solution-cards blocks entirely (no separate "Solutions/解决方案" line above the heading). The block's contentEn now holds the JSON card array; the eyebrow text previously lived there. Rather than relocate the eyebrow (bilingual issue) or special-case non-JSON content, the simplest spec-compliant path is to render only the heading `title` ("Where We Help / 应用领域") — which is already bilingual and self-descriptive. The admin no longer has an eyebrow field for this block type, which matches the spec.
- Backward-compat: an empty or invalid contentEn still AUTO-PULLS the L1 categories (useCategories hook) so a fresh install / never-edited block keeps rendering 4 cards. The PageManager editor falls back to mapping L1 categories to editable rows so the admin sees what they edit.
- Lint: 0 errors / 0 warnings. Seed: clean. No `bun run build` performed, no tests written.

---
Task ID: 19
Agent: main (application-area cards self-editable in PageManager)
Task: Make the home "应用领域" cards fully editable in PageManager (card images/names/descriptions stored in the block itself, not on categories)

Work Log:
- Root cause of "换不了图片": the solution-cards block AUTO-PULLED cards from L1 categories (name/desc/image lived on the Category model), so the card images were only editable in CategoryManager — not where the admin sees the block (PageManager). The category image upload API itself worked (verified PUT -> 200 + stored), it was a discoverability/ownership mismatch.
- Fix (delegated to subagent Task 18): made solution-cards SELF-CONTAINED. The block's contentEn now holds a JSON array of cards [{nameEn,nameCn,descEn,descCn,image,linkCategoryId?}]. SolutionCardsBlock parses it (falls back to auto-pulling L1 categories when empty, for backward-compat). PageManager BlockDialog gets a SolutionCardsEditor (repeatable rows: bilingual name/desc + ImageUploader per card + optional link-category dropdown). Seeded the 4 cards as JSON.
- Verified: home solution-cards block contentEn is a JSON array of 4 cards (each with image+desc). Edit test: PUT the block's contentEn (changed a card's image + nameCn) -> 200, change stored. Lint 0 errors. Re-seeded clean.

Stage Summary:
- The admin can now edit each "应用领域" card's IMAGE / name / description directly in PageManager (edit the solution-cards block) — no more going to CategoryManager. "换不了图片" fixed.
- Cards keep the click-to-category behavior (optional linkCategoryId per card).
- Backward-compat: an empty solution-cards block still auto-pulls L1 categories.
- Pushed.
